...
type oidcClient interface {
	...
	Exchange(ctx context.Context, code, verifier, nonce string) (clinks.ExternalIdentity, error)
}

type oidcSessionService interface {
	CurrentSession(ctx context.Context, token string) (clinks.Session, error)
	LoginExternal(ctx context.Context, identity clinks.ExternalIdentity) (clinks.Session, error)
	LinkExternalIdentity(ctx context.Context, session clinks.Session, identity clinks.ExternalIdentity) error
	AcceptExternalInvitation(ctx context.Context, invitationToken string, identity clinks.ExternalIdentity, locale clinks.Locale) (clinks.Session, error)
}
...
func (server *Server) googleOIDCStart(client oidcClient, config OIDCConfig) stdhttp.HandlerFunc {
	return func(w stdhttp.ResponseWriter, r *stdhttp.Request) {
		...
		case "link":
			...
			if _, err := server.auth.sessions.CurrentSession(r.Context(), token); err != nil {
				server.oidcFailure(w, r, config)
				return
			}
...
func (server *Server) googleOIDCCallback(client oidcClient, config OIDCConfig) stdhttp.HandlerFunc {
	return func(w stdhttp.ResponseWriter, r *stdhttp.Request) {
		...
		switch attempt.Mode {
		case "link":
			var session clinks.Session
			session, err = auth.CurrentSession(r.Context(), server.cookieToken(r.Header))
			if err == nil {
				err = auth.LinkExternalIdentity(r.Context(), session, identity)
			}
			if err == nil {
...
func (server *Server) hasPasswordVerified(r *stdhttp.Request) bool {
	if len(server.auth.oidcSecret) < 32 {
		return false
	}
...
func (server *Server) passwordVerificationMAC(sessionToken, expiresUnix string) []byte {
	key := sha256.Sum256([]byte("clinks:password-verification:v1\x00" + server.auth.oidcSecret))
