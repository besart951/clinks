...
type PasswordHasher struct {
	cost         int
	rejectedHash domain.PasswordHash
}
...
func NewPasswordHasherWithConfig(
	...
	return &PasswordHasher{
		cost:         cost,
		rejectedHash: domain.PasswordHash(rejectedHash),
	}, nil
}

func (hasher *PasswordHasher) Hash(
	password string,
) (domain.PasswordHash, error) {
	if len(password) > 72 {
		return "",
			fmt.Errorf(
				"hash password: %w",
...
func (hasher *PasswordHasher) NeedsRehash(hash domain.PasswordHash) bool {
	if hash == "" {
		return false
	}
