...
func (repository *Store) ListRoles(
	ctx context.Context,
	...
	if filter.Sort == domain.RoleSortCreatedAt {
	...
	if filter.Cursor != "" {
		...
		if err != nil {
			return domain.Page[domain.Role]{}, domain.NewError(domain.ErrorValidation)
		}
	...
	var page domain.Page[domain.Role]
...
func (repository *Store) FindRole(
	ctx context.Context,
	tenantID domain.TenantID,
...
func (repository *Store) PermissionsForRole(
	ctx context.Context,
	tenantID domain.TenantID,
	roleID domain.RoleID,
) ([]domain.Permission, error) {
	role, err := repository.FindRole(ctx, tenantID, roleID)
	if err != nil {
		return nil, err
	}
...
func (repository *Store) CreateRole(
	ctx context.Context,
	role domain.Role,
	actorID domain.UserID,
) (domain.Role, error) {
	role.Name = strings.TrimSpace(role.Name)
	role.Kind = domain.RoleKindCustom
	if role.ID == "" {
		id, err := newUUID()
		if err != nil {
	...
	err := WithTenantTx(ctx, repository.pool, role.TenantID, func(tx pgx.Tx) error {
...
func (repository *Store) UpdateRole(
	ctx context.Context,
	role domain.Role,
	actorID domain.UserID,
) (domain.Role, error) {
...
func (repository *Store) DeleteRole(
	ctx context.Context,
	tenantID domain.TenantID,
	roleID domain.RoleID,
	revision uint64,
...
func createDefaultRoles(
	...
	administrator, err := newRole(tenantID, administratorRoleName, domain.RoleKindAdministrator, domain.AllPermissions())
	if err != nil {
		return defaultRoles{}, err
	}
	user, err := newRole(tenantID, userRoleName, domain.RoleKindUser, domain.DefaultUserPermissions())
	if err != nil {
		return defaultRoles{}, err
	}

	roles := defaultRoles{Administrator: administrator, User: user}
	for _, role := range []domain.Role{roles.Administrator, roles.User} {
		if err := insertRole(ctx, tx, role); err != nil {
			return defaultRoles{}, err
		}
...
func insertRole(ctx context.Context, tx pgx.Tx, role domain.Role) error {
	_, err := tx.Exec(ctx, `
...
func findRoleTx(
	...
	if err == pgx.ErrNoRows {
		return domain.Role{}, domain.NewError(domain.ErrorRoleNotFound)
	}
	if err != nil {
		return domain.Role{}, fmt.Errorf("find role: %w", err)
	}
	return role, nil
}
...
func scanRole(row pgx.CollectableRow) (domain.Role, error) {
	return scanRoleValue(row)
}

func scanRoleValue(row rowScanner) (domain.Role, error) {
	var role domain.Role
	...
	for index, permission := range permissions {
		role.Permissions[index] = domain.Permission(permission)
	}
	return role, nil
}
