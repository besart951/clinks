...
func (client *GoogleOIDC) Exchange(
	...
	verifier,
	nonce string,
) (clinks.ExternalIdentity, error) {
	if !client.Enabled() {
		return clinks.ExternalIdentity{}, ErrOIDCDisabled
	}
	...
	if idToken.Nonce != nonce {
		return clinks.ExternalIdentity{}, ErrInvalidTokenClaims
	}
	...
	email, err := clinks.ParseEmail(claims.Email)
	if err != nil {
		return clinks.ExternalIdentity{},
	...
	return clinks.ExternalIdentity{
		Issuer:  googleIssuer,
		Subject: clinks.ExternalSubject(idToken.Subject),
		Email:   email,
	}, nil
}
