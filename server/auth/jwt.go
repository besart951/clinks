// Package auth owns session signing, password hashing, invitation tokens, and OIDC.
package auth
...
func (issuer *SessionIssuer) Verify(
	...
	claim := clinks.SessionClaim{
		UserID:         clinks.UserID(claims.Subject),
		SessionVersion: claims.Version,
	}

	if claims.TenantID != "" {
		claim.ActiveTenantID = new(
			clinks.TenantID(claims.TenantID),
		)
	}
...
func invalidSessionClaim() (
	...
	return clinks.SessionClaim{},
		clinks.NewError(clinks.ErrorInvalidCredentials)
}
